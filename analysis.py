...
class Analysis:
    ...
    def var(self, headers, rows=[]):
        ...
        NOTE: Loops are forbidden!
        '''
        return np.var(self.data.select_data(headers, rows),axis = 0)
    ...
    def std(self, headers, rows=[]):
        ...
        NOTE: Loops are forbidden!
        '''
        std =  np.var(self.data.select_data(headers, rows),axis = 0) ** 0.5
    ...
    def pair_plot(self, data_vars, fig_sz=(15, 15), title=''):
        ...
        fig, ax = plt.subplots(len_data_vars, len_data_vars, figsize = fig_sz, sharex = 'col', sharey = 'row')

        fig.suptitle(title)

        #loop through each row and each col depending on number of variables in data_vars
        for row in range(0, len_data_vars):
            for col in range(0, len_data_vars):
                #Depending on a given row and col, put plot on right position in grid
                ax[row,col].scatter(self.data.select_data([data_vars[col]]), self.data.select_data([data_vars[row]]))
                # set labels
                plt.setp(ax[-1,col], xlabel = data_vars[col])
                plt.setp(ax[row,0], ylabel = data_vars[row])
